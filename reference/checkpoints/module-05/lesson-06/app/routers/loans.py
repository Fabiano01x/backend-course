...
from app.schemas import ErrorResponse, LoanCreate, LoanResponse
...
@router.get(
    ...
    description="Lista fatos do empréstimo sem carregar relações implicitamente.",
    response_description="Empréstimos em ordem de criação.",
)
async def list_loans(session: DatabaseSession) -> list[LoanResponse]:
    loans = await LoanRepository(session).list_all()
    return [to_loan_response(loan) for loan in loans]
