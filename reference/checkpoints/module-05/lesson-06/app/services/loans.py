...
async def borrow_book(session: AsyncSession, payload: LoanCreate) -> Loan:
    repository = LoanRepository(session)
    try:
        async with session.begin():
            user = await repository.find_user(payload.user_id)
            if user is None:
                raise LoanRuleError("Usuário não encontrado", not_found=True)
            if not user.active:
                raise LoanRuleError("Usuário inativo não pode retirar livros")
