...
class ConnectionContext:
    ...
    async def run_sync(self, operation: Callable[..., Any]) -> None:
        self.ran_create_all = (
            operation.__self__ is Base.metadata
            and operation.__name__ == "create_all"
        )
...
async def test_lifespan_creates_tables_and_disposes_engine() -> None:
    engine = RecordingEngine()
    database = make_database(engine)
    application = create_app(Settings(_env_file=None), database)

    async with application.router.lifespan_context(application):
        assert engine.connection.ran_create_all is True
