...
@pytest.mark.parametrize(
    ...
    [
        ([Result(rows=[])], "Credencial de acesso inválida", 401),
        (
            [Result(rows=[(user(1, active=False), "member")])],
            "Credencial de acesso inválida",
            401,
        ),
        (
            [Result(rows=[(user(1), "member")]), Result()],
...
async def test_borrow_requires_current_member_assignment(
    client: AsyncClient, session: RecordingSession
) -> None:
    session.execute_results.append(
        Result(rows=[(user(1), "librarian")])
    )
...
async def test_lists_and_returns_a_loan(
    ...
    returned = await client.post(
        "/loans/1/return", headers=authorization()
    )
...
async def test_return_requires_current_librarian_assignment(
    client: AsyncClient, session: RecordingSession
) -> None:
    session.execute_results.append(Result(rows=[(user(1), "member")]))
