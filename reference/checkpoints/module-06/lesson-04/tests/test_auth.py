...
async def test_registers_normalized_email_and_persists_only_hash(
    ...
    assert [item.role_name for item in stored.role_assignments] == ["member"]
...
async def test_valid_credentials_issue_a_short_access_token(
    ...
    raw_refresh_token = response.cookies[REFRESH_COOKIE_NAME]
    stored_refresh_token = session.added[0]
    assert isinstance(stored_refresh_token, RefreshToken)
    assert stored_refresh_token.user_id == 1
    assert stored_refresh_token.token_digest == digest_refresh_token(
        raw_refresh_token
    )
    assert raw_refresh_token not in response.text
    assert raw_refresh_token not in stored_refresh_token.token_digest
