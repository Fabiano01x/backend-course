...
def test_declares_library_entities_and_referential_integrity() -> None:
    ...
    assert "create table oidc_login_attempts" in sql
...
def test_oidc_schema_links_by_issuer_subject_and_stores_only_digests() -> None:
    identities = table_body(schema_text(), "external_identities")
    attempts = table_body(schema_text(), "oidc_login_attempts")

    assert "unique (issuer, subject)" in identities
    assert "references users (id) on delete cascade" in identities
    assert "browser_digest varchar(64) not null" in attempts
    assert "state_digest varchar(64) not null" in attempts
    assert "nonce_digest varchar(64) not null" in attempts
    assert "verifier_digest varchar(64) not null" in attempts
    assert "code_verifier" not in attempts
