...
def test_revision_history_has_one_auditable_head() -> None:
    ...
    assert head == "0006_api_keys"
    assert revision is not None
    assert revision.down_revision == "0005_oidc_identities"
    assert revision.doc == "Cria identidades de máquina e chaves de API revogáveis."
...
def test_upgrade_and_downgrade_can_be_rendered_without_database() -> None:
    ...
    assert "CREATE TABLE external_identities" in rendered_upgrade
    assert "CREATE TABLE oidc_login_attempts" in rendered_upgrade
    assert "CREATE TABLE api_clients" in rendered_upgrade
    assert "CREATE TABLE api_keys" in rendered_upgrade
    assert "JSON" in rendered_upgrade
    assert "DROP TABLE api_keys" in rendered_downgrade
    assert "DROP TABLE api_clients" in rendered_downgrade
    assert "DROP TABLE oidc_login_attempts" in rendered_downgrade
    assert "DROP TABLE external_identities" in rendered_downgrade
