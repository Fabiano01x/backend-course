...
from app.dependencies import CurrentIdentity, LibrarianPrincipal
...
def translate_rule_error(error: LoanRuleError) -> HTTPException:
    ...
    if error.forbidden:
        return HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail=error.detail
        )
...
@router.post(
    ...
    responses={
        401: {"model": ErrorResponse, "description": "Access token inválido."},
        403: {"model": ErrorResponse, "description": "Papel member ausente."},
...
async def finish_loan(
    loan_id: int,
    session: DatabaseSession,
    identity: CurrentIdentity,
) -> LoanResponse:
    try:
        loan = await return_book(
            session, loan_id, user_id=identity.user_id
        )
    except LoanRuleError as error:
        raise translate_rule_error(error) from error
    return to_loan_response(loan)
