...
ApiScope = Literal["books:read", "loans:read"]


class ApiClientCreate(StrictSchema):
    name: str = Field(
        min_length=2,
        max_length=120,
        description="Nome estável do serviço, parceiro ou rotina.",
    )
...
class ApiClientResponse(StrictSchema):
    id: UUID
    name: str
    active: bool
    created_at: datetime
...
class ApiKeyIssueRequest(StrictSchema):
    scopes: set[ApiScope] = Field(
        min_length=1,
        max_length=2,
        description="Capacidades mínimas concedidas à credencial.",
    )
    expires_at: datetime | None = Field(
        default=None,
        description="Expiração opcional; quando informada, deve estar no futuro.",
    )

    @field_validator("expires_at")
    @classmethod
    def expiration_must_be_aware_and_future(
        cls, value: datetime | None
    ) -> datetime | None:
        if value is None:
            return None
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("expires_at deve informar fuso horário")
        if value <= datetime.now(UTC):
...
class ApiKeyRotateRequest(StrictSchema):
    expires_at: datetime | None = Field(
        default=None,
        description="Nova expiração; omitir cria a substituta sem expiração.",
    )
    ...
    def expiration_must_be_aware_and_future(
        ...
        return ApiKeyIssueRequest.expiration_must_be_aware_and_future(value)


class ApiKeyResponse(StrictSchema):
    id: UUID
    client_id: UUID
    prefix: str
...
class ApiKeyIssueResponse(ApiKeyResponse):
    api_key: str = Field(
        description="Segredo exibido somente nesta resposta; não pode ser recuperado."
    )
