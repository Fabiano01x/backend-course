...
async def exercise_http_contract(settings: Settings) -> None:
    ...
    async with application.router.lifespan_context(application):
        async with AsyncClient(
            ...
            user_detail = await client.get(f"/users/{user_id}")
            due_at = (datetime.now(UTC) + timedelta(days=14)).isoformat()
            concurrent_loans = await asyncio.gather(
                client.post(
                    "/loans",
                    json={"user_id": user_id, "book_id": book_id, "due_at": due_at},
                ),
                client.post(
                    "/loans",
                    json={"user_id": user_id, "book_id": book_id, "due_at": due_at},
            ...
            protected = await client.delete(f"/books/{book_id}")
            ...
            deleted = await client.delete(f"/books/{disposable.json()['id']}")
    ...
    assert valid_login.status_code == 204
