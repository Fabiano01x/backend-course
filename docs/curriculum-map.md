...
| Ordem original | Aula original | Fonte | Conceitos identificados | Nova posição | Nova aula | Status | Justificativa |
|---:|---|---|---|---:|---|---|---|
| 1 | JWT Authentication with Access and Refresh Tokens | `source/module-06/01.md` | JWT, claims, access token, refresh token, login, armazenamento no cliente | 1 e 2 | Identidade local sem armazenar senhas; Access tokens curtos e identidade autenticada | A01 e A02 concluídas, divididas e corrigidas | A fonte pressupõe um `authenticate_user` inexistente. M06/A01 adiciona Argon2id, migração compatível, cadastro e verificação genérica. M06/A02 emite apenas access JWT curto com algoritmo fixo, claims e contexto validados; a retirada deriva o usuário do token. Refresh não foi estático nem antecipado antes de seu ciclo seguro. |
| 2 | Secure Refresh Token Management | `source/module-06/02.md` | rotação, `jti`, persistência, revogação, reutilização, logout | 3 | Sessões renováveis sob um navegador hostil | Concluído, integrado e corrigido | Rotação e proteção do transporte pertencem ao mesmo problema. M06/A03 armazena digest de token opaco, família e estado transacional; replay concorrente revoga a família. Não usa token bruto nem uma tabela chamada blocklist que também funcionaria como allowlist. |
| 3 | Implementing RBAC Systems | `source/module-06/03.md` | AuthN versus AuthZ, roles, associação muitos-para-muitos, claims, dependências, `403` | 4 | Autorização explícita com papéis | Concluído, integrado e corrigido | M06/A04 cria `roles` e `user_roles`, consulta atribuições atuais em toda operação protegida e comprova revogação com o mesmo JWT. Claims de papel não são autoritativas; propriedade do próprio perfil e papéis globais permanecem regras distintas. |
| 4 | Implementing OAuth2 for Social Login | `source/module-06/04.md` | Authorization Code, redirect URI, state, client secret, callback, login social | 5 | Login social é OpenID Connect | Concluído, integrado e corrigido | M06/A05 usa OIDC Authorization Code com state, nonce, PKCE S256, discovery, RS256/JWKS e sessão local. Identidades são ligadas por `(issuer, subject)`; e-mail verificado só cria conta nova e nunca faz auto-link de conta existente. |
| 5 | API Key Authentication for Service-to-Service Communication | `source/module-06/05.md` | credencial de máquina, header, comparação segura, hash, prefixo, chamada entre serviços | 6 | Chaves de API com ciclo de vida | Concluído, integrado e corrigido | M06/A06 separa `ApiClient` de `ApiKey`, usa prefixo público, segredo de 256 bits exibido uma vez, digest, escopos, expiração, auditoria, rotação e revogação. Rotas humanas e de integração mantêm credenciais distintas; TLS continua obrigatório. |
| 6 | Preventing XSS and CSRF Attacks | `source/module-06/06.md` | XSS, encoding, cookies, CSRF, SameSite, token, Fetch Metadata, defesa em profundidade | 3 | Sessões renováveis sob um navegador hostil | Concluído, antecipado, integrado e corrigido | O risco nasce quando o refresh token entra em cookie. M06/A03 combina `HttpOnly`, `Secure` em produção, `SameSite=Strict`, header customizado, CORS e `Origin`, e explicita que essas camadas não previnem execução XSS. Exemplos Flask/Jinja não foram atribuídos à arquitetura FastAPI atual. |
