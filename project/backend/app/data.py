"""Estado temporário em memória; será substituído no módulo de banco."""

from itertools import count

from app.schemas import BookCreate, BookResponse, UserCreate, UserResponse


books: dict[int, BookResponse]
users: dict[int, UserResponse]
_book_ids: count
_user_ids: count


def reset_data() -> None:
    """Restaura dados determinísticos, inclusive entre testes."""

    global books, users, _book_ids, _user_ids
    books = {
        1: BookResponse(
            id=1,
            title="Clean Architecture",
            author="Robert C. Martin",
            isbn="9780134494166",
            available=True,
        )
    }
    users = {
        1: UserResponse(
            id=1,
            name="Ada Lovelace",
            email="ada@example.com",
            active=True,
        )
    }
    _book_ids = count(2)
    _user_ids = count(2)


def create_book(data: BookCreate) -> BookResponse:
    book = BookResponse(id=next(_book_ids), available=True, **data.model_dump())
    books[book.id] = book
    return book


def create_user(data: UserCreate) -> UserResponse:
    user = UserResponse(id=next(_user_ids), active=True, **data.model_dump())
    users[user.id] = user
    return user


reset_data()

